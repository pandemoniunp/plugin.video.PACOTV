...
resources = xbmc.translatePath(os.path.join('special://home/addons/plugin.HarryTV/resources', ''))
...
def onhockey(params):
    plugintools.log("[HarryTV 0.3.0].Onhockey")

    plugintools.add_item(action="", title= '[COLOR red][B] O N  [/COLOR][COLOR blue] H O C K E Y[/COLOR][COLOR white]  T V[/COLOR]   [/B][COLOR lightyellow][I]By [/I][/COLOR]', url = "", thumbnail = thumbnail , fanart = fanart, folder = False, isPlayable = False)
...
def multihockey(params):
    plugintools.log("[HarryTV-0.3.1].multihockey "+repr(params))
    ...
    if data.strip() != "":
        ...
        try:
            ...
            if selector == 0:
                ...
                #xbmc.executebuiltin("Notification(%s,%s,%i,%s)" % ('HarryTV', "Sin enlaces disponibles", 3 , art+'icon.png'))
            if selector == 1:
...
def url_onhockey(url,tipo):
    plugintools.log("[HarryTV-0.3.0.URL_onHockey "+url)
    plugintools.log("tipo?= "+tipo)
...
def gethttp_headers(url):
    plugintools.log("HarryTV-0.3.0.gethttp_referer_headers "+url)
    request_headers=[]
